...
// TestRelay_SubgroupBeforeSubscribeOKIsDelivered: a publisher may open a
// track's subgroup streams as soon as it accepts the SUBSCRIBE, and they can
// reach the relay before the SUBSCRIBE_OK that names their Track Alias. §11.4.2
// lets the receiver "buffer it for a brief period to handle reordering with the
// control message that establishes the Track Alias"; abandoning it loses the
// first Groups of the track. The SUBSCRIBE_OK here is sent only once the relay
// is holding the stream (or, for a relay that abandons it, after a second).
//
// Not parallel: the hook is process-wide, and no parallel test runs alongside
// a serial one.
func TestRelay_SubgroupBeforeSubscribeOKIsDelivered(t *testing.T) {
	...
	if _, err := subSess.Subscribe(t.Context(), &message.Subscribe{Namespace: ns, Name: []byte("cam1")}); err != nil {
	...
	if !fetchesObject(t, dialAnotherClient(t, pubSess), ns, []byte("cam1"), 2*time.Second) {
...
// fetchesObject FETCHes the track from the relay until the response carries
// the Object the test published (payload "x"), or within runs out.
func fetchesObject(
	t *testing.T,
...
// TestRelay_EarlySubgroupWaitIsBounded pins both limits on holding a subgroup
// stream for an unknown Track Alias: at most maxEarlyStreams wait per session,
// and none waits past earlyAliasWait. Without them a peer could pin the relay's
// flow control with aliases it never binds.

...
func TestSubscription_BornEstablished(t *testing.T) {
	...
	for _, sub := range []interface {
		...
		registry.NewUpstreamSub(1, nil, nil, 0, 0),
...
func TestSubscription_TerminateLatch(t *testing.T) {
	t.Parallel()
	sub := registry.NewUpstreamSub(1, nil, nil, 0, 0)
...
func TestUpstreamSub_FilterRoundTrip(t *testing.T) {
	t.Parallel()
	sub := registry.NewUpstreamSub(7, nil, nil, 42, 0)
	if sub.GetFilter() != nil {
		t.Fatal("initial filter not nil")
	}
	f := &message.LocationFilter{Fields: 2}
	sub.SetFilter(f)
...
func TestDownstreamSub_AccessorsRoundTrip(t *testing.T) {
	...
	f := &message.LocationFilter{Fields: 2, StartGroup: 5, StartObject: 3}
	sub.SetFilter(f)
	sub.SetPriority(64)
	sub.SetGroupOrder(1)
